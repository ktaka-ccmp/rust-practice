...
fn main() {
    ...
    let client_session_key = derive_session_key(client_shared.raw_secret_bytes(), b"tls13-session-key");
    let server_session_key = derive_session_key(server_shared.raw_secret_bytes(), b"tls13-session-key");

    assert_eq!(client_session_key, server_session_key, "セッション鍵が一致しない！");
    println!("セッション鍵確立: {}... (PFS達成)", hex::encode(&client_session_key[..12]));
    ...
    let nonce1 = Nonce::from_slice(&iv1);
    let encrypted_server_msg = cipher.encrypt(nonce1, server_message.as_bytes()).unwrap();
    println!("サーバー: Certificate+CertificateVerify を暗号化して送信: {}",
    ...
    let decrypted_bytes = cipher_c.decrypt(nonce1, encrypted_server_msg.as_slice()).unwrap();
    ...
    let parsed_ca_sig = Signature::from_der(
        &B64.decode(extract_json_field(&decrypted_msg, "caSignature")).unwrap()
    ).unwrap();
    ca_verifying
    ...
    // 証明書からサーバーの公開鍵を取り出す
    let pub_hex = extract_json_field(&decrypted_msg, "publicKey");
    ...
    let parsed_cv_sig = Signature::from_der(
        &B64.decode(extract_json_field(&decrypted_msg, "certVerifySignature")).unwrap()
    ).unwrap();
    let transcript_bytes =
        B64.decode(extract_json_field(&decrypted_msg, "handshakeTranscript")).unwrap();
...
fn extract_json_field(json: &str, key: &str) -> String {
    ...
    // publicKeyはネストされた構造内にあるので別途処理
    let pattern2 = format!(r#""{}":"#, key);
    if let Some(start) = json.find(&pattern2) {
        let rest = &json[start + pattern2.len()..];
